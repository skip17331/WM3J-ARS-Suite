...
public class LivePassPanel extends VBox {
    ...
    public LivePassPanel(ServiceRegistry services) {
        ...
        satNameLabel = styledLabel("— NO SATELLITE SELECTED —", "#b6c2cf", true, fz + 1);

        // ── Big AZ / EL display ──────────────────────────────────────────────
        azBigLabel   = styledLabel("---", "#4fc3f7", true, fz + 13);
        elBigLabel   = styledLabel("---", "#69f0ae", true, fz + 13);
        azUnitLabel  = styledLabel("AZ", "#5d6a78", false, fz - 3);
        elUnitLabel  = styledLabel("EL", "#5d6a78", false, fz - 3);
        ...
        countdownLabel = styledLabel("—",  "#5d6a78", true,  fz + 3);
        passLine1Label = styledLabel("",   "#93a0ae", false, fz - 1);
        passLine2Label = styledLabel("",   "#93a0ae", false, fz - 1);
        Label passHeader = styledLabel(com.hamradio.jsat.i18n.I18n.get("panel.pass.prediction"), "#6f7d8c", false, fz - 3);
        ...
        Label rrHeader   = styledLabel(com.hamradio.jsat.i18n.I18n.get("panel.range.rate"),   "#6f7d8c", false, fz - 3);
        Label rangeKey   = styledLabel("Range",  "#6f7d8c", false, fz - 3);
        Label rateKey    = styledLabel("Rate",   "#6f7d8c", false, fz - 3);
    ...
    public VBox buildPolarCard(int fz) {
        Label header = styledLabel(com.hamradio.jsat.i18n.I18n.get("panel.polar.plot"), "#6f7d8c", false, fz - 3);
        ...
        card.setStyle("-fx-background-color: rgba(13,16,32,0.88); -fx-background-radius: 6; "
                    + "-fx-border-color: #323b47; -fx-border-radius: 6; -fx-border-width: 1; "
    ...
    public void update() {
        ...
        sunlitLabel.setStyle(sunlitLabel.getStyle().replaceAll("-fx-text-fill: [^;]+;",
            "-fx-text-fill: " + (state.inSunlight ? "#ffdd00" : "#5d6a78") + ";"));
        ...
        if (elDeg <= 0) {
            passText  = "below horizon"; passColor = "#5d6a78";
        } else if (elDiff > 0.01) {
    ...
    private void drawPolarPlot(GraphicsContext gc, SatelliteState state) {
        ...
        Color dotColor = state.elevationDeg > 0
            ? Color.web("#4fc3f7") : Color.web("#5d6a78");
        gc.setFill(dotColor);
    ...
    private void updateCountdown(SatelliteState state) {
        ...
        if (passes == null || passes.isEmpty()) {
            ...
            countdownLabel.setStyle(countdownLabel.getStyle().replaceAll("-fx-text-fill: [^;]+;", "-fx-text-fill: #5d6a78;"));
    ...
    private static String cardStyle() {
        return "-fx-background-color: #1b2027; -fx-background-radius: 4; "
    ...
    private static javafx.scene.layout.Region separator() {
        ...
        sep.setStyle("-fx-background-color: #323b47; -fx-min-width: 1; -fx-max-width: 1; -fx-min-height: 40;");
    ...
    private static Label key(String text, int fz) {
        return styledLabel(text, "#6f7d8c", false, fz - 3);
    }

    private static Label val(String text, int fz) {
        return styledLabel(text, "#dde3ea", true, fz - 2);
    }
