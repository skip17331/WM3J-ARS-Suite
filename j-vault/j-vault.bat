...
set "JAR="
for /f "delims=" %%F in ('dir /b /a-d /o-d "%SCRIPT_DIR%target\j-vault-*.jar" 2^>nul ^| findstr /v /i "sources javadoc shaded fat"') do if not defined JAR set "JAR=%SCRIPT_DIR%target\%%F"
if not defined JAR (
