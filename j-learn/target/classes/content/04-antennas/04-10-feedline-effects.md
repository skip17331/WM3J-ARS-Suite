...
For other lengths, the impedance traces a **circle on the Smith chart** (see §04-09), which is why two operators with the same antenna and different feedline lengths can report wildly different SWR readings at the radio.
...
The J-Hub feedline calculator (see §07) handles this; manual calculation: physical length = (492 × VF) / f(MHz) for an electrical half-wave.
