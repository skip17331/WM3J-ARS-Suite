...
- **Bold rows** are chapters (clickable to open the chapter overview, e.g. §07-00).
- **Indented rows** are sections within a chapter (e.g. §07-02 MPE Limits).
...
- **`balun`** to find every section mentioning balun in the title (§05-12, §22-05).
- **`21-`** to filter to chapter 21 only.
- **`emcomm`** to find chapter 26.
...
| **§08 Antenna Workshop** | 📡 Antenna Workshop | Opens the matching calculator panel in the J-Hub Antenna Workshop tab |

The banner appears for every section in the chapter, not just the overview. From §08-10 (Yagi-Uda) the banner button takes you straight to the Yagi calculator pre-filled with example values.
...
4. **§05-00** — Antennas overview (then jump to whatever interests you)
5. **§07-00** — RF Safety (you're legally responsible for this — read once, refer back)
6. **§13–17** — Troubleshooting cluster (skim; come back when something breaks)
7. **§18-00** — Shack Inventory (set up your station's records)
8. **§24-00** — Band Plans (operating-frequency reference; bookmark)
...
- §08-01 — Antenna Recommender (navigates antennas backward from your QTH)
- §03-09 — Hardware Keyer Builds (cross-references the trainer's hardware module)
- §18-00 — Shack Inventory (where the suite stores your station record)
