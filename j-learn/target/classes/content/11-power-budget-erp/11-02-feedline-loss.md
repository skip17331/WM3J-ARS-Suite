...
This section is the budget-perspective treatment — how to compute the loss and apply it to the budget. The conceptual treatment is in §09 (Feedline & SWR) and the per-cable reference values are in §22-02.
...
|------|-----------|--------|
| Matched-line loss | Loss when SWR = 1:1; pure conductor + dielectric heating | §22-02 (per-cable table) × L/100 |
| Mismatch loss | Extra loss from reflected wave bouncing through lossy line | §09-03 (table by matched-loss × SWR) |
...
(All matched-line loss; assumes SWR 1:1. For higher SWR, add mismatch loss from §09-03's table.)
...
- Mismatch loss (from §09-03 lookup, 0.5 dB matched + SWR 1.5:1): **~0.05 dB**.
...
- Mismatch loss: from §09-03 lookup, 6 dB matched + SWR 4:1 → ~3 dB extra.
- Total: 9 dB.
...
- §22-02 — Coax loss tables (full reference)
- §09-01 — Coax loss by frequency (the conceptual treatment)
- §09-03 — Mismatch loss (the SWR penalty)
- §09-04 — Power delivered vs. lost
