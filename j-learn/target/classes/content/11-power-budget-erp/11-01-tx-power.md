...
- **At fold-back**: under high SWR, the rig automatically reduces power. A "100 W" rig at SWR 3:1 may actually be running at 50 W. (See §09-02.)
...
For SSB voice, the rig is typically modulated 30-50%, so average power is 30-50% of peak. For digital modes (FT8, RTTY), duty cycle is 100% and average = peak. See §07-04 for the duty-cycle factors that affect time-averaged power for RF safety calculations.
...
For thermal budgets (rig PA heatsink, feedline heating, antenna components), use average. For RF exposure (§07-04), use time-averaged. For "what reaches the antenna in this moment", use peak (which equals rated if the rig isn't folded back).
...
- §09-02 — SWR & reflected power (fold-back trigger)
- §20-01 — Battery maintenance (for portable power supply)
