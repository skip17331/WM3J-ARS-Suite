...
- **Note the noise floor** on each band you use. A 6+ dB increase from baseline indicates a new noise source somewhere (yours or someone else's; see §14).
...
- **Coax routing**. Walk the run from antenna to entry point. Look for:
  ...
  - Coax loops near house electrical (creates RFI risk; see §12)
...
- **Check firmware notification feeds** for your major equipment. Note any new releases since last quarter. Read release notes and decide whether to update (see §17-02).
...
- **Replace any visibly degraded coax**. UV-cracked jacket means the dielectric below it is at risk. See §17-04.
...
- **Battery capacity test**. Run the annual load test (per §17-01).
...
- **Document**. Photograph everything. Update the inventory (§15). Date the inspection.
...
- §17-00 — Overview (this is part of the broader maintenance discipline)
- §17-04 — Coax replacement (when an inspection turns into a replacement)
