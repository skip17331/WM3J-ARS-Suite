...
- **Indicator switches to TX but no audio:** PTT is fine; mic audio is the problem (see §11-03).
...
- §11-02 — no receive (often diagnosed with the same tools)
- §11-03 — distorted audio (related — TX works but sounds bad)
- §11-06 — power supply (the source of many "no TX" problems)
- §10 — high SWR (which can also produce "no TX" via fold-back protection)
