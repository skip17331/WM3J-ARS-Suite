...
- **Indicator switches to TX but no audio:** PTT is fine; mic audio is the problem (see §13-03).
...
- §13-02 — no receive (often diagnosed with the same tools)
- §13-03 — distorted audio (related — TX works but sounds bad)
- §13-06 — power supply (the source of many "no TX" problems)
- §12 — high SWR (which can also produce "no TX" via fold-back protection)
