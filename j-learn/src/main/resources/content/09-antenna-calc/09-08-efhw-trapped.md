...
A trapped EFHW combines the **end-fed half-wave** topology (§09-07) with **trap circuits** (§09-13) to create a multi-band antenna in **less physical space** than a non-trapped EFHW. Each trap, resonant at one band, isolates a section of the wire above that band; below the trap's resonance the trap loads the antenna inductively.
...
A trapped EFHW already fits more bands into less wire than the harmonic-only version (§09-07). A shortened design shrinks it further by re-specing the traps with higher L / lower C (same f_r) and adding an inline seg-1 loading coil when needed.

### Mechanism

Same physics as the shortened trapped dipole (§09-05): the trap's below-resonance reactance is
...
- New builders — start with a non-trapped EFHW (§09-07) to learn the unun + counterpoise + trim workflow first
...
1. **Build all traps first** using §09-13 Trap Design. Verify each on a NanoVNA.
...
- §09-07 — EFHW (No Traps) — simpler, harmonic-only variant
- §09-13 — Trap Design
