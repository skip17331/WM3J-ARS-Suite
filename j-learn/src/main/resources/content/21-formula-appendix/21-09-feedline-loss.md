...
The SWR penalty depends on both matched loss and SWR; see §09-03 (mismatch loss) for the closed-form.
...
- **Treating SWR as a separate loss to add.** Matched loss and SWR-mismatch loss interact: a high-loss cable hides SWR (because the reflected wave is attenuated). See §09-03.
...
- §22 — Coax & Connectors (full chapter on cable selection)
- §22-02 — Loss tables (more cable types)
- §09-03 — Mismatch loss (the SWR penalty)
- §11-02 — Feedline loss in the budget chain
- §21-10 — Decibels (the dB ↔ power-factor conversion)
