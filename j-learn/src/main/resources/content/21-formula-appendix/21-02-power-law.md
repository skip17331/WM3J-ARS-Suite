...
The relationship between voltage, current, resistance, and power. Combines Ohm's Law (§21-01) with the definition of power. Comes up every time you size a power supply, fuse, dummy load, resistor, or PA stage.
...
- §11 — Power Budget & ERP (the chain from PA output through feedline to ERP)
- §13 — High SWR (uses P = I²R to derive heating in mismatched lines)
- §21-01 — Ohm's Law (Ohm's Law combined with P = V·I gives the full set)
- §21-08 — ERP (uses the same dB-style power math)
- §21-10 — Decibels (the log-domain way to manipulate power ratios)
