---
id: 21-14
title: Formula Calculator (RF Exposure Worked Example)
chapter: 21
section: 14
level: mixed
status: draft
...
# Formula Calculator — RF Exposure Worked Example

> **Advanced callout convention:** sections or paragraphs intended for Extra-class / engineering depth are marked with a blockquote starting `> ⚙️ **Advanced —**`. Simple-mode renderers can hide these; advanced-mode renderers show them inline.

This is the closing section of the Formula Appendix. It does two things:

1. Sketches the **Formula Calculator** pattern — the user-facing tool that plugs into every formula card in this chapter.
2. Walks through the **RF exposure** calculation end-to-end as the rich example, since it pulls together more variables (power, frequency, duty cycle, gain, distance) than any other formula in the appendix.

The formula calculator is the "lazy ham mode" version of this whole chapter — type the numbers in, get the answer out, with the underlying formula visible so you can sanity-check the result. The RF exposure walk-through below shows what every section of the calculator UI looks like in practice.

## The formula calculator pattern

Every formula card in this chapter (Ohm's Law, Wavelength, SWR, ERP, etc.) maps cleanly into a uniform UI:
...
│                                                          │
│ Result:    EIRP = 398 W                                  │
│            (also: 25.99 dBW, 56 dBm)                     │
│                                                          │
│ Sanity check: <inline reasoning>                         │
│ See also:    §21-08, §07                                 │
└─────────────────────────────────────────────────────────┘
```

Three architectural choices that make this work as an in-app tool:

1. **Unit pickers per input.** No "convert MHz to Hz" footguns. The user picks units, the calculator does the conversion under the hood.
2. **Multiple result formats.** ERP shown as W, dBW, and dBm at once. The user picks the form they need.
3. **A linked sanity check.** Every result includes a one-line plausibility statement ("This is roughly 4× the rig output, consistent with 6 dB gain"). The number alone is a hazard; the number with a sanity check is a tool.

The actual implementation lives outside this chapter (it's a J-Hub web UI panel that pulls from the same formula metadata). What follows is the calculation it would perform for the most demanding common case: RF exposure.

## RF exposure — what's being calculated
...
The relevant chapter for the regulatory side is §07 (RF Safety). This section is the math.
...
| Feedline loss | L | 0.5 dB … 10 dB | dB | §21-09 |
...
(Linear feedline-loss factor; see §21-09 for the conversion.)
...
(See §21-08 for the dBi/dBd conversion.)
...
If S_worst ≤ S_MPE, you're compliant. If not, you must either reduce power, increase distance, reduce duty cycle, or change antenna placement (§07).
...
L = 1 dB              (LMR-400, 100 ft, 14 MHz — see §21-09)
G = 6 dBd → 8.15 dBi
...
- §07 — RF Safety (the regulatory side; the *why*)
- §07-02 — MPE Limits (the table this section uses)
- §07-04 — Duty Cycle (how to estimate D for your operating style)
- §07-06 — Safe Antenna Placement (how to fix a failing scenario)
- §11 — Power Budget & ERP (related calculation chain)
- §21-08 — ERP / EIRP (the formula card this builds on)
- §21-09 — Feedline Loss (where L comes from)
- §21-10 — Decibels (the dB ↔ linear conversions used throughout)
