...
|-------|---------------------|----------------|
| **Recommender** | "What antenna should I build?" | §08-01 + UI wizard |
| **Calculators** | "Given antenna type X, what dimensions?" | Each per-antenna section + UI calc panels |
| **Trim workflow** | "I built it; what does the sweep tell me?" | §08-15 |
...
The chapter pairs with a J-Hub web UI tab — **Antenna Workshop** — that implements the actual calculations and the recommender wizard. Every chapter section that documents a calculator includes a **▶ Launch in calculator** button (same pattern as §03 → Morse Trainer) that jumps to the matching panel in the UI.
...
- **Not** an antenna theory chapter — that's **§05 Antennas**, which explains how each type works, polarization, ground effects, modeling. This chapter assumes you've decided what to build (or are using the recommender to decide) and need the numbers.
...
All formulas live in the chapter cards and reference back to **§21 Formula Appendix** for the foundational math:

| When you need... | Look at... |
|------------------|-----------|
| Half-wave length, quarter-wave length | §21-06 Wavelength |
| Resonant L-C for traps | §21-05 Resonant Frequency |
| Reactance of loading coils | §21-03 Reactance |
| Smith chart math for matching networks | §21-13 Smith Chart Basics |
| Q calculation for traps and mag loops | §21-11 Q Factor |
| Loss budget through feedline | §21-09 Feedline Loss |

The §21 cards have the full derivations; §08 cards have the *applied* math (with end-effect corrections, height factors, etc.) for each antenna type.
...
Plan to **trim** every antenna after installation. The calculators give you a starting length; the analyzer tells you the truth. §08-15 walks through the trim workflow.
...
- §05 — Antennas (theory chapter, deep dives on each antenna family)
- §13 — High-SWR troubleshooting (when the trim sweep doesn't match the calculator)
- §21 — Formula Appendix (foundational math)
- §22 — Coax & Connectors (feedline choice — affects what your antenna sees)
