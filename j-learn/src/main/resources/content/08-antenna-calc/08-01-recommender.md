...
- Score every antenna in §08-02 through §08-15 against the answers.
...
Each antenna in §08-02 to §08-15 has a small **profile** that says, for each questionnaire dimension, the conditions under which it's viable / preferred / impossible. The recommender computes a score per antenna, ranks them, and explains the score.
...
- §05 — Antennas (full theory for each type the recommender suggests)
- §08-02 through §08-15 — calculator cards for each antenna
- §11 — Power Budget & ERP (relevant once you've chosen and need to compute coverage)
- §13 — High-SWR troubleshooting (post-build, if results don't match prediction)
