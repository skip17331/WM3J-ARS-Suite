...
Packet radio is a connection-oriented digital protocol — **AX.25** — running over RF. Stations *connect* to each other in a TCP-like handshake, exchange acknowledged frames, and disconnect. Beneath that, AX.25 is **the same protocol** that APRS (§03-05) uses; the difference is that APRS is connectionless broadcasts, while packet sets up explicit point-to-point sessions.
...
- §03-00 — Digital modes overview
- §03-05 — APRS (uses AX.25, same protocol, different application)
- §04 — Repeaters & Bandplans (where the packet sub-bands are)
- §21 — Emergency communications (where packet still earns its keep, especially WinLink)
