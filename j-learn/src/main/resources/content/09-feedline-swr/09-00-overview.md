...
This chapter covers the relationship between SWR (Standing Wave Ratio), feedline loss, and what actually arrives at your antenna. The math here is the foundation of antenna-system design and is referenced by §05-10 (feedline effects on antenna behavior), §11 (power budget), §13 (high-SWR troubleshooting), and §22 (coax & connectors reference).
...
You have a 100 W transmitter, 100 ft of RG-8X feeding a 20 m dipole. Look up matched-line loss in §22-02: RG-8X at 14 MHz is 1.0 dB/100 ft. So the dipole sees about 100 W − 1.0 dB = 79 W in the perfect case (1:1 SWR).

Now suppose the antenna is mismatched, presenting 3:1 SWR. Looking up §09-03's mismatch loss table: an additional ~0.5 dB. Total loss is 1.5 dB. The dipole sees 100 W − 1.5 dB = 71 W.
...
- **Per-cable specs (loss/100 ft, VF, dimensions)** — see §22 for the complete reference table.
- **Connector types and how to install them** — see §22-04.
- **Baluns and chokes** — see §05-12 (full discussion) and §22-05 (reference).
- **Specific feedline failure modes** — see §13-01 (coax issues) and §13-07 (water ingress).
- **Antenna behavior** — see §05.
...
- **§08 (Antenna calculator)** uses these formulas to compute physical lengths.
- **NanoVNA / antenna analyzer** — the practical tool for measuring SWR at multiple frequencies.

## See also

- §05-10 — Feedline effects on antenna behavior
- §05-11 — Impedance transformation (companion to §09-06)
- §11 — Power budget and ERP (uses these numbers in system-level context)
- §13 — High-SWR troubleshooting (when something goes wrong)
- §22 — Coax & connectors reference
