...
This section is the budget-perspective treatment — how to compute the loss and apply it to the budget. The conceptual treatment is in §08 (Feedline & SWR) and the per-cable reference values are in §16-02.
...
|------|-----------|--------|
| Matched-line loss | Loss when SWR = 1:1; pure conductor + dielectric heating | §16-02 (per-cable table) × L/100 |
...
(All matched-line loss; assumes SWR 1:1. For higher SWR, add mismatch loss from §08-03's table.)
...
- Mismatch loss (from §08-03 lookup, 0.5 dB matched + SWR 1.5:1): **~0.05 dB**.
...
- Mismatch loss: from §08-03 lookup, 6 dB matched + SWR 4:1 → ~3 dB extra.
- Total: 9 dB.
...
- §16-02 — Coax loss tables (full reference)
