...
A **full-wave loop** is a closed wire about one wavelength in circumference, hung in any closed shape — square (quad), triangle (delta), or larger irregular shapes (skyloop). Unlike the small magnetic loop in §04-05, this loop is electrically *large*; its current and voltage distributions span a full wave around its perimeter. It radiates as a normal-sized antenna, not via near-field magnetic coupling.
