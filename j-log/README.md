...
└── src/main/
    ...
    ├── java/com/j-log/
    │   ├── app/
    │   │   ├── JLogApp.java          ← JavaFX Application entry point
...
Three SQLite databases are created at `~/.j-log/` on first run:
...
1. Bundled plugins in the JAR (`resources/com/j-log/plugins/*.json`)
2. User-installed plugins in `~/.j-log/plugins/*.json`
...
Logs are written to `~/.j-log/logs/j-log.log` with daily rotation (7 days retained).

---

## Data Files

All data is stored in `~/.j-log/`:
...
2. Place it in `~/.j-log/plugins/` **or** import via
   **Contest → Import Plugin...** from the contest chooser.
3. Restart J-Log (or the plugin loader will detect it on next chooser open).
