# HamClock Clone — Java/JavaFX

A cross-platform desktop amateur radio operator dashboard replicating and extending the HamClock application.
...
| Persistent JSON settings (~/.hamclock/settings.json) | ✅ |
...
All settings are saved to `~/.hamclock/settings.json` and applied **instantly** to the running display without restart.
...
```
src/main/java/com/hamclock/
├── app/
│   ├── HamClockApp.java          ← Entry point (JavaFX Application)
