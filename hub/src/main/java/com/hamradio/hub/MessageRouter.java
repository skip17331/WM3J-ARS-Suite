 ...
 *
 * The router is a singleton wired by HubMain.
 */
public class MessageRouter {
    ...
    // Late-bound reference to the WS server (set by HubMain after server starts)
    private HubServer hubServer;

    public void setHubServer(HubServer server) {
        this.hubServer = server;
    }

    // ---------------------------------------------------------------
    // Main dispatch method — called by HubServer.onMessage
    // ---------------------------------------------------------------
     ...
     */
    public void route(HubServer.AppSession session,
                      JsonObject msg,
                      String rawJson,
                      HubServer server) {

        this.hubServer = server; // keep reference current
    ...
    private void handleRigStatus(JsonObject msg, String rawJson,
                                 WebSocket sender, HubServer server) {
        try {
    ...
    private void handleSpotSelected(String rawJson, WebSocket sender, HubServer server) {
        try {
            ...
            //   • hamlog populates its entry bar
            //   • hamclock populates its DX window
            server.broadcastToAll(rawJson);

            // Also derive and broadcast RIG_STATUS so the app that owns CI-V/CAT
            // (hamlog) tunes the rig to the correct frequency and mode.
            if (freq > 0) {
    ...
    private void handleWsjtxDecode(String rawJson, WebSocket sender, HubServer server) {
        server.broadcastToAll(rawJson);
    ...
    public void publishRawLine(String line) {
        if (hubServer != null) {
            ...
            hubServer.broadcastToAppName("webconfig", msg.toString());
        }
    }
    ...
    public void publishSpot(Spot spot) {
        StateCache.getInstance().addSpot(spot);
        if (hubServer != null) {
            hubServer.broadcastToAll(ConfigManager.gson().toJson(spot));
        }
    }
    ...
    public void onAppDisconnected(String appName) {
        ...
        // The app list broadcast is handled by HubServer.onClose
    }
}
