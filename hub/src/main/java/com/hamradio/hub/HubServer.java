...
/**
 * HubServer — WebSocket server that manages all connected application sessions.
 *
 * Each connecting app must immediately send an APP_CONNECTED registration
 * message.  After registration the hub:
 ...
 */
public class HubServer extends WebSocketServer {
    ...
    public HubServer(InetSocketAddress address, MessageRouter router, StateCache cache) {
        super(address);
        this.router = router;
        this.cache  = cache;
        setReuseAddr(true);
    ...
    public void onClose(WebSocket conn, int code, String reason, boolean remote) {
        ...
        if (session != null) {
            ...
            if (getConnectedCount() == 0) {
                log.info("Last app disconnected — shutting down hub");
                System.exit(0);
            }
        }
    }
    ...
    @Override
    public void onStart() {
        log.info("HubServer WebSocket listener started");
    }
    ...
    private void handleRegistration(AppSession session, JsonObject msg) {
        ...
        ack.addProperty("type", "HUB_WELCOME");
        ack.addProperty("hubVersion", "1.0.0");
