...
let ws         = null;  // live WebSocket connection to j-hub
...
function populateForm(cfg) {
  ...
  val('wsPort',         cfg.jHub?.websocketPort  || 8080);
  val('webPort',        cfg.jHub?.webConfigPort  || 8081);
  ...
  // App Launcher
  val('launchCmdJMap',    cfg.apps?.jMap?.command       || '');
  check('launchAutoJMap', cfg.apps?.jMap?.autoLaunch    ?? false);
  val('launchCmdJLog',       cfg.apps?.jLog?.command       || '');
  check('launchAutoJLog',    cfg.apps?.jLog?.autoLaunch    ?? false);
  val('launchCmdJBridge',    cfg.apps?.jBridge?.command    || '');
  check('launchAutoJBridge', cfg.apps?.jBridge?.autoLaunch ?? false);
  val('launchCmdJDigi',      cfg.apps?.jDigi?.command      || '');
  check('launchAutoJDigi',   cfg.apps?.jDigi?.autoLaunch   ?? false);
}
...
function collectConfig() {
  ...
  config.apps = {
    jMap: {
      command:    val('launchCmdJMap'),
      autoLaunch: document.getElementById('launchAutoJMap').checked,
    },
    jLog: {
      command:    val('launchCmdJLog'),
      autoLaunch: document.getElementById('launchAutoJLog').checked,
    },
    jBridge: {
      command:    val('launchCmdJBridge'),
      autoLaunch: document.getElementById('launchAutoJBridge').checked,
    },
    jDigi: {
      command:    val('launchCmdJDigi'),
      autoLaunch: document.getElementById('launchAutoJDigi').checked,
    },
  };
...
// ---------------------------------------------------------------
// WebSocket — live updates from j-hub
// ---------------------------------------------------------------

function connectWebSocket() {
  const wsPort = parseInt(document.location.port) - 1 || 8080; // guess
  // Attempt to connect to j-hub WS port (default 8080)
  try {
...
// ---------------------------------------------------------------
// J-Map settings
// Settings are persisted in j-hub.json via /api/jmap.
// When J-Map is running at localhost:8082, live settings are
// fetched from there first; on save they are pushed to both.
// ---------------------------------------------------------------

const HC_LIVE_API = 'http://localhost:8082/api/settings';
const HC_HUB_API  = '/api/jmap';

async function fetchJMap() {
  const badge = document.getElementById('jMapStatusBadge');
  let s    = null;
  let live = false;

  // 1. Try the live J-Map app
  try {
    const res = await fetch(HC_LIVE_API, { signal: AbortSignal.timeout(2000) });
    if (res.ok) { s = await res.json(); live = true; }
  } catch (_) {}

  // 2. Fall back to j-hub-stored settings
  if (!s) {
    try {
...
async function saveJMap() {
  const cfg = collectJMap();
  const el  = document.getElementById('jMapSaveMsg');
  let hubOk  = false;
  let liveOk = false;

  // Always persist to j-hub
  try {
  ...
  // Also push to live J-Map if reachable
  try {
...
// Fetch J-Map settings when the tab is clicked
document.addEventListener('DOMContentLoaded', () => {
  document.querySelectorAll('.tab[data-tab="jMap"]').forEach(btn => {
    btn.addEventListener('click', fetchJMap);
  });
});
...
/**
 * Convert a hyphenated app name to camelCase for DOM element ID lookups.
 * Examples: 'j-log' → 'jLog', 'j-bridge' → 'jBridge', 'j-digi' → 'jDigi'
 */
...
async function fetchAppStatus() {
  try {
    ...
    setAppBadge('jMap',     s.jMap);
    setAppBadge('j-log',    s['j-log']);
    setAppBadge('j-bridge', s['j-bridge']);
    setAppBadge('j-digi',   s['j-digi']);
  } catch (_) {}
}

function setAppBadge(name, running) {
  const el = document.getElementById(nameToId(name) + 'RunBadge');
  if (!el) return;
...
async function appLaunch(name) {
  const msgEl  = document.getElementById(nameToId(name) + 'LaunchMsg');
  const idName = nameToId(name);
  const cmdId  = 'launchCmd' + idName.charAt(0).toUpperCase() + idName.slice(1);
